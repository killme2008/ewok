 ...
 * <p>
 * On-disk format has been implemented following Mike Spille's recommendations.
 * Quoting him:
 * </p>
 * 
 * <i>
 * <p>
 * <code>[RECORD_TYPE :4] [RECORD_LEN :4] [HEADER_LEN :4] [System.currentTimeMillis :8] [Sequence number :4]
 * [Checksum :4] [Payload :X] [END_RECORD_INDICATOR :4]</code>
 * </p>
 * <p>
 * Where [RECORD_TYPE] is a passed-in record type from the TM. [RECORD_LEN] is
 * the overall record length (sans [RECORD_TYPE and [RECORD_LEN]). [HEADER_LEN]
 * is the length of the remainder of the header - important if you want to
 * support easy upgrades of your format. The remaining pieces are the rest of
 * the header, and the payload. The header at least should have
 * [System.currentTimeMillis] and [Sequence number], with the [sequence number]
 * coming from some monotically increasing sequence generator unique to the
 * process. The [checksum] is optional for the paranoid among us. The time
 * information can be very useful for profiling and tracking down problems in
 * production, and in conjunction with the sequence number it can give you
 * precise ordering. This doesn't give you much in this solution, but can be
 * priceless if you ever move to a system with multiple dual log file pairs to
 * lessen single-threading on a single log file pair. Finally, I like having an
 * [END_RECORD_INDICATOR] as an extra corruption detector device - I'm a
 * suspenders and belt kind of guy. Actually, the END_RECORD_INDICATOR and
 * [RECORD_LEN] in conjunction are very useful in development, as well, to catch
 * programming mistakes in the log system early.
 * </p>
 * </i>
 * 
 * <p>
 * Payload contains
 * <code>[GTRID LENGTH :1] [GTRID :A] [UNIQUE NAMES COUNT :4] ([UNIQUE NAME LENGTH :2] [UNIQUE NAME :Y] ...)</code>
 * which makes a major difference with Mike's proposed format because here a
 * record can vary in length: the GTRID size is A bytes long (A being the GTRID
 * length) and there can be X unique names that are Y characters long, Y being
 * eventually different for each name.
 * </p>
 * 
 * @see <a
 *      href="http://jroller.com/page/pyrasun?entry=xa_exposed_part_iii_the">XA
...
public class TransactionLogRecord {
     ...
     * same size. Very useful when debugging and eventually restoring broken log
     * files.
     */
    ...
    public TransactionLogRecord(int status, int recordLength, int headerLength, long time, int sequenceNumber,
            int crc32, Uid gtrid, Set<String> uniqueNames, int endRecord) {
        this.status = status;
     ...
     * {@link #calculateCrc32()}) and compare it with the stored value.
     * 
     * @return true if the recalculated value equals the stored one, false
     *         otherwise.
     */
    ...
    int calculateTotalRecordSize() {
        return calculateRecordLength(uniqueNames) + 4 + 4; // + status + record
                                                           // length
    }
    ...
    private int calculateRecordLength(Set<String> uniqueNames) {
        ...
        for (String uniqueName : uniqueNames) {
            totalSize += 2 + uniqueName.length(); // 2 bytes for storing the
                                                  // unique name length + unique
                                                  // name length
        }
    ...
    /**
     * Length of all the fixed size fields part of the record length header
     * except status and record length.
     * 
     * @return fixedRecordLength
     */
    private int getFixedRecordLength() {
        return 4 + 8 + 4 + 4 + 1 + gtrid.getArray().length + 4 + 4; // record
                                                                    // header
                                                                    // length +
                                                                    // current
                                                                    // time +
    ...
    private int getRecordHeaderLength() {
        return 4 + 4 + 4 + 8 + 4 + 4; // status + record length + record header
                                      // length + current time + sequence number
                                      // + checksum
    }
